...
| `liststate` | Posts `EVT_USB_LISTSTATE`; the UI task prints one machine-readable line for harness assertions: `[LISTSTATE] {"screen":N,"refresh_state":N,"pill":0\|1,"count":N,"cache_age_s":N,"auto_retry":N,"selected":N,"latch":0\|1,"armed":0\|1,"scroll_y":N,"pill_hiding":0\|1}` (`cache_age_s` = -1 when unknown — no cache or no valid wall clock; `selected` = `shopping_list_scroll_idx`; `latch` = `shopping_list_touch_pull_consumed` — 1 after a completed refresh means the pull-to-refresh latch is wedged; `armed` = `shopping_list_touch_pull_armed`; `scroll_y` = `lv_obj_get_scroll_y(shopping_list_scroll)`, 0 if the container is null, negative = pulled below the top; `pill` = border refresh ring active/visible and `pill_hiding` = ring fade-out in progress — the field NAMES are kept from the old pill indicator for harness compatibility). |
