...
- **`RefreshState`**: REFRESH_IDLE, REFRESH_WAKE_PENDING, REFRESH_INFLIGHT, REFRESH_COMPLETE, REFRESH_FAILED
  ...
  - **UI_LIST dedup must respect the SM**: the dedup window (`LCD_UI_LIST_DEDUPE_MS` = 1000ms since `lcd_last_ui_list_complete_ms`) only drops a UI_LIST when **nothing** is expecting a list: `!((refresh_state == REFRESH_WAKE_PENDING || refresh_state == REFRESH_INFLIGHT) || lcd_refresh_inflight || waiting_for_list_response)`. A refresh triggered within ~1s of a completed one gets a cached-serve answer in ~100-300ms — before `lcd_refresh_inflight` is set for the new cycle — so gating on `lcd_refresh_inflight` alone discarded the new answer as a duplicate and the SM spun to the 20s hard timeout.
  - On the shopping list, a **unified 20s hard watchdog** (`REFRESH_HARD_TIMEOUT_MS` = 20000) covers **both** REFRESH_INFLIGHT (measured from `lcd_refresh_start_ms`) **and** REFRESH_WAKE_PENDING (measured from `refresh_wake_pending_start_ms`) — so ANY non-IDLE refresh resolves to IDLE by 20s. Both call `refresh_hard_timeout_clear()` (reason `inflight_hard_timeout` / `wake_pending_hard_timeout`, incrementing `refresh_timeout_count`) — clears all inflight flags with **no auto-retry** (unlike `refresh_soft_fail`), posts `EVT_REFRESH_TIMEOUT` so the UI task stops the glow, re-renders the cached list, and plays the border-ring **error flash** + transient "Couldn't refresh" toast (`shopping_list_ring_show_error()`). Lets the 10s idle-sleep engage instead of spinning forever. (20s also stops killing legit ~12s slow-WiFi refreshes that the old 12s cap aborted.)
  ...
  - On the shopping list, WAKE_PENDING/INFLIGHT are surfaced as the **border refresh ring** (a ~70° hero-green segment sweeping around the screen edge — no text, list stays fully usable) — see the Shopping List Screen section; the UI task polls `shopping_list_refresh_indicator_sync()` each iteration since the WAKE_PENDING -> INFLIGHT transition happens on the UART task.
- **Shopping-list keep-awake (`LIST_ACTIVE`)**: `uart_send_list_active(bool)` in lcd_uart.h tells the Sense to stay awake + keep WiFi up. Sent `true` on entering the list and re-asserted ~every 3s from `loop()`; sent `false` on leaving (both via `ui_show_screen` enter/leave detection) and on LCD idle-sleep entry (`enterLightSleep`) if still on the list. The ~3s re-assert keeps `true` while a refresh is in flight too (`idle_age < INACTIVITY_TIMEOUT_MS || refresh_state == REFRESH_WAKE_PENDING || REFRESH_INFLIGHT`) so the Sense doesn't sleep mid-fetch even if the user is otherwise idle.
...
Round-display layout (360x360 circular panel — corners don't exist):
- **Title**: single label centered at the top arc (`LV_ALIGN_TOP_MID`, y=34), montserrat_20, hero green `0x1F4D2B`, with the item count integrated: "Shopping List • 6" (separator is the bullet U+2022 — the built-in Montserrat fonts do not include U+00B7).
- **Cards**: centered column inside a 300x230 scroll viewport at (30, 60). Card geometry constants: `SHOPPING_LIST_CARD_W` 272 (clears the circle at all visible heights), `SHOPPING_LIST_CARD_H` 60 (fixed), `SHOPPING_LIST_CARD_RADIUS` 16, `SHOPPING_LIST_CARD_GAP` 10. Item text montserrat_18, single line, `LV_LABEL_LONG_DOT`, left-mid aligned. Selected = white bg + 4px hero-green left accent bar (`LV_BORDER_SIDE_LEFT`) + `LV_SYMBOL_RIGHT` arrow prefix + soft tan shadow; unselected = flat tan `0xD4C4AE`, no shadow. Scrollbar OFF (a straight scrollbar fights the round bezel).
- **Edge fades**: `shopping_list_add_edge_fade()` stacks four 8px input-transparent strips of the screen bg (`0xF5E9D8`, opa 255/165/95/40) at the top (y 60-92) and bottom (y 258-290) of the viewport so rows dissolve at the circle's narrow zones instead of hard-clipping (LVGL 8 styles have no opacity gradients). Scroll pads: top 24 / bottom 40 so first/last rows rest clear of the fades.
- **Back control**: circular 56px hero-green button bottom-center (`LV_ALIGN_BOTTOM_MID`, -10 → y≈294-350) with a white `LV_SYMBOL_LEFT` (montserrat_20). Handled by the manual tap path in `shopping_list_handle_touch` (hitbox y>=288, x 130-230) — replaced the old rectangular back row at y=318 that sat partially outside the visible circle.
- **Empty state** (genuinely-empty only, post-refresh): centered `LV_SYMBOL_LIST` glyph (montserrat_32, tan), "No items on your list" (montserrat_18, `0x1A1A1A`), and a muted "Pull down to refresh" hint (montserrat_14, `0x888888`). Pre-first-refresh count==0 still shows only the neutral hint.

Behavior:
- **Stale-while-revalidate entry**: `show_shopping_list_screen_impl()` renders whatever `g_active` holds INSTANTLY (non-empty after the first ever fetch thanks to the NVS cache), then ALWAYS auto-triggers a background refresh (`shopping_list_trigger_refresh("entry_revalidate")`) unless one is already pending/inflight. The cached list stays fully interactive (scroll/selection/delete) while it runs. **The entry-revalidate refresh runs SILENTLY — NO spinning ring on list-open** (clean entry); only a USER pull-to-refresh (touch pull / encoder overscroll / USB) shows the ring. See the gating note under "Border refresh ring". The old explicit wake in `SHIP_MENU_ACTION_SHOPPING_LIST` / USB `list` was removed — entry covers it.
...
- **Skeleton loader**: `shopping_list_render_skeleton()` — four placeholder cards (272x60, radius 16, `0xE7D9C3` light tan) with a gentle opacity pulse (lv_anim COVER→OPA_40, 450ms + 450ms playback ≈ 900ms period, infinite repeat, 110ms stagger). The next `populate()` starts with `lv_obj_clean()`, which deletes the placeholders and kills their anims the moment real content arrives.
- **Touch pull-to-refresh**: `shopping_list_scroll` has `LV_OBJ_FLAG_SCROLL_ELASTIC` + `LV_OBJ_FLAG_SCROLL_MOMENTUM`, and `shopping_list_scroll_event_cb()` (PRESSED/SCROLL/RELEASED/SCROLL_END) tracks elastic overscroll past the top. Pulling down >`SHOPPING_LIST_PULL_HINT_PX` (12px) shows the **progressive pull cue** — the border ring fills clockwise from 12 o'clock with pull progress (0→360° at `SHOPPING_LIST_PULL_THRESHOLD_PX` = 45px) via `shopping_list_ring_show_pull(progress_pct, armed)`. At/past the threshold the ring is full with a width 5→7 emphasis bump (armed); release fires `shopping_list_trigger_refresh("touch_pull")` exactly once per gesture (`shopping_list_touch_pull_consumed` latch), at which point the SM sync takes over with the sweep. Scroll gestures also set `shopping_list_touch_was_scrolled` so the raw tap path (`shopping_list_handle_touch`) doesn't treat a drag as a tap and pop the item overlay.
- **Gesture latch re-arm rules**: the once-per-gesture latch (`shopping_list_touch_pull_consumed`) re-arms in FOUR places, and all are required: (1) `LV_EVENT_SCROLL_END` with `scroll_y >= 0` — the normal path when the elastic snap-back settles; (2) `shopping_list_screen_populate()` immediately after `lv_obj_clean()` — **critical**, because a fast refresh (~600ms) completes mid-elastic-snap-back and `lv_obj_clean` interrupts LVGL 8.4's scroll-end delivery: the emptied container never receives `LV_EVENT_SCROLL_END`, so without this reset the latch stays consumed forever and every subsequent touch pull is silently rejected (the `refresh_busy || consumed` guard in `shopping_list_scroll_event_cb` eats SCROLL, the RELEASED path eats the fire). Populate also forces `lv_obj_scroll_to_y(shopping_list_scroll, 0, LV_ANIM_OFF)` after the clean (before rows are re-added, so it can't fight the `scroll_to_view` selection restore) in case the rebuild lands with `scroll_y` still negative mid-pull; (3) `show_shopping_list_screen_impl()` on screen entry (belt-and-braces, the screen is rebuilt from scratch anyway); (4) **on refresh RESOLVE** via `shopping_list_reset_pull_latch()` (lcd_ship_screens.h) called from the UI task in BOTH the `EVT_REFRESH_TIMEOUT` handler **and** the `EVT_LIST_REPLACED` completion path — covers a refresh that resolves **without** repopulating: a hard-timeout that never lands a list, or an unchanged-list completion (`new_sig == shopping_list_rendered_sig`) that skips `shopping_list_screen_populate()` and therefore its built-in reset (2). Without this, those resolve paths left the latch consumed and the next pull-to-refresh was silently rejected.
...
- **Border refresh ring** (replaced the floating refresh pill): one input-transparent `lv_arc` hugging the screen edge, built by `shopping_list_build_refresh_ring()` as the LAST child of the screen (top layer; `LV_OBJ_FLAG_CLICKABLE` cleared so it never blocks touches). Constants: `SHOPPING_LIST_RING_SIZE` 352, `SHOPPING_LIST_RING_WIDTH` 5 (7 when pull-armed), `SHOPPING_LIST_RING_SWEEP_DEG` 70, `SHOPPING_LIST_RING_SWEEP_MS` 1200 (one revolution). Track invisible (arc_opa transp on MAIN), indicator hero green `0x1F4D2B`, rounded ends, no knob, rotation 270 so angles measure from 12 o'clock. **Ring shown only for user pulls (silent entry-revalidate)**: `shopping_list_refresh_show_ring` (file-scope bool) gates the ring. `shopping_list_trigger_refresh(reason)` sets it fresh on EVERY trigger BEFORE the `indicator_sync(true)` call: `false` for `entry_revalidate` (silent auto-refresh on list-open — clean entry, no spinner), `true` for everything else (`touch_pull`, encoder `list_refresh`, `usb_pull`/`usb_refresh`). In `shopping_list_refresh_indicator_sync()` the WAKE_PENDING/INFLIGHT show-sweep is wrapped in `if (shopping_list_refresh_show_ring)`, and REFRESH_FAILED's `show_error()` is likewise gated (it does NOT early-return on a hidden ring, so a failed silent entry-revalidate must not flash/toast). `finish()` and `hide()` already early-return on a hidden ring, so they stay ungated and are no-ops for a silent refresh. `shopping_list_refresh_indicator_sync()` (same name/state-diffing flow as the pill era; UI task polls it every iteration, `show_shopping_list_screen_impl()` forces it on entry) maps the SM: WAKE_PENDING/INFLIGHT -> `shopping_list_ring_show_sweep()` (one infinite linear lv_anim rotating the 70° segment — the only continuous animation, and only while refreshing, **and only when `shopping_list_refresh_show_ring`**); REFRESH_COMPLETE -> `shopping_list_ring_finish()` (segment closes to a full 360° ring over 260ms ease-out, brief hold, 300ms fade); REFRESH_FAILED / `EVT_REFRESH_TIMEOUT` -> `shopping_list_ring_show_error()` (full ring in the family error red `0x8B1E2D` flashes twice ~500ms, fades, plus a transient "Couldn't refresh" toast near the top arc that auto-hides after ~2.5s — errors are the only text); IDLE -> quick 150ms fade-hide (`shopping_list_ring_hide()`, no-op while a finish/error sequence owns the hide via `shopping_list_ring_autohide`, and skipped mid-press so the poll can't fight the pull-progress fill). `shopping_list_ring_hiding` guards double-hides and is reported as `pill_hiding` in `liststate`.
...
- **Header-aware selection scroll (2026-06-17)**: scrolling onto the FIRST item of a store group used to clip its group header off the top — `lv_obj_scroll_to_view` aligned the card to the top, pushing the header (positioned just above it) out of view, so after scrolling down then back up the top header was hidden. Fix: a parallel `shopping_list_item_headers[50]` array tracks each first-in-group item's header object (NULL otherwise); `shopping_list_add_store_header()` now RETURNS the created header (`lv_obj_t*` instead of void) and `shopping_list_screen_populate()` resets the array at the top and captures `shopping_list_item_headers[i] = <hdr>` when it inserts a header before item i. Both scroll-to-selection sites — `shopping_list_update_selection(old,new)` (knob scroll, `LV_ANIM_ON`) and the initial scroll-to-selected at the end of `shopping_list_screen_populate()` (`LV_ANIM_OFF`) — pick `shopping_list_item_headers[idx]` as the scroll target when present, else the card, so a first-in-group selection always reveals its header. Bounds-guarded (idx within 0..49) and NULL-target-guarded; delete/index logic untouched.
- **Store-group headers (2026-06-17)**: the Sense now sends each UI_LIST item with a `store` field (e.g. "Whole Foods", "" = none) and sorts items so same-store items are CONTIGUOUS (empty store last). `app_state_t` carries a parallel `stores[MAX_LIST_ITEMS][48]` array (index-aligned with `items[]`/`item_ids[]`), copied alongside items everywhere they are copied/shifted/swapped (UART_RX parse + optimistic preserve/voice-insert paths in lcd_uart_rx.h, the local delete shifts in lcd_ship_screens.h + lcd_menu.h, and the NVS save/load JSON `stores` array in lcd_persist.h — the whole-struct swap `g_active = g_pending` carries it automatically). `shopping_list_add_store_header(store)` creates a small, dim (`0x9A8C76`), uppercased, LEFT-aligned montserrat_14 label as a NON-clickable separate flex child in `shopping_list_scroll` — it is NOT stored in `shopping_list_items[]` and does NOT consume an item index. In `shopping_list_screen_populate()`, before creating each item card, a header is inserted whenever the store changes from the previous item (`i==0 || strcmp(stores[i],stores[i-1])!=0`); the trailing empty-store group is labelled **"Other"**. Index mapping is unchanged: `shopping_list_items[i] = card` still maps item index i → its card, so selection highlight, `shopping_list_delete_index(idx)`, and `lv_obj_scroll_to_view` stay item-index-based; the flex column auto-positions headers between groups. `store` is folded into `shopping_list_content_sig()` so a store-only change still rebuilds (and re-renders headers).
- **List idle → Home before sleep (2026-06-17)**: when the device sits idle on `SCREEN_SHOPPING_LIST` and reaches the home sleep threshold (`home_age_ms >= HOME_SLEEP_DELAY_MS`), the main loop's sleep-eligibility block (LCD_Minimal.ino) returns to Home BEFORE darkening/sleeping — so the device sleeps on Home and wakes on Home (no longer re-shows the list on wake). It does this cross-core-safely by setting `provision_return_home_pending = true` (the UI task on Core 1 consumes it → `show_ship_main_menu()` → `ui_screen_state = SCREEN_HOME`; no LVGL touched from Core 0), calls `uart_send_list_active(false)` so the Sense knows the list closed, resets `home_shown_ms = 0` so Home gets its own full sleep delay, logs `[LIST] idle timeout -> return home before sleep`, and `goto loop_continue` to skip this iteration. Fires once per idle period: once the UI task switches to HOME, later iterations see HOME and fall through to the normal home-sleep path. The actively-on-list case (`home_age_ms < delay`) is untouched.
- **Content-signature render skip**: `shopping_list_content_sig()` (FNV-1a over count + item texts + ids + store) is stored in `shopping_list_rendered_sig` by every populate (0xFFFFFFFF = placeholder/skeleton sentinel that never matches). The `EVT_LIST_REPLACED` handler compares the new `g_active` sig against it and **skips the rebuild entirely** when identical — no staggered reveal, no flicker on revalidate-with-no-changes (the common case).
...
- **Delete/Back overlay**: tapping a list item opens `shopping_list_show_overlay()` — a 50%-black full-screen backdrop with a centered white card (280px wide, min-height 140, pad_hor 12 / pad_ver 20) holding the wrapped item name (montserrat_16, width 240) and a 254x48 button row with **Delete** (red `0xE53935`) and **Back** (green `0x1F4D2B`) buttons, each 120x48 with a 14px gap, labels montserrat_18. The overlay is `lv_obj_move_foreground`'d after build, and the refresh ring / error toast re-foreground it via `shopping_list_keep_overlay_on_top()` whenever they self-foreground, so it always renders above the ring and edge fades.
- **Overlay hitbox derivation (correct by construction)**: the overlay branch of `shopping_list_handle_touch()` does NOT use hard-coded coordinates. `shopping_list_show_overlay()` stores the card/button objects in `shopping_list_overlay_card` / `shopping_list_overlay_delete_btn` / `shopping_list_overlay_back_btn` and calls `lv_obj_update_layout(overlay)` so coords are final before the first tap; the touch handler then hit-tests `lv_obj_get_coords()` rectangles inflated by 8px slop, in priority order: **delete button → back button → inside card (deliberate no-op, so a slightly missed button never dismisses the overlay) → outside card (dismiss)**. **The overlay "Back" button only dismisses the overlay and STAYS on the list (2026-06-17)** — it no longer calls `show_ship_second_menu()` (that was a bug that navigated away from the list); leaving the list is the dedicated circular bottom-center back button (the no-overlay `y>=288, x 130-230` path). Overlay-back logs `-> dismiss (stay on list)`. Layout/restyle tweaks can never desync hitboxes from pixels again (the old magic numbers put the button row at y>=195 while it actually rendered ~y=172-216 depending on label wrap, so Delete taps fell into dead zones). Every overlay tap logs its classification: `[SHOPPING_LIST] overlay tap (x,y) -> delete|back|card|dismiss`. First-tap safety: `shopping_list_show_overlay()` clears `shopping_list_touch_was_scrolled` (the drag-suppression flag is only consumed by the no-overlay branch, so a programmatic open could otherwise carry a stale flag), nothing sets `touch_ignore_until` on overlay open, and the scroll event cb early-returns while the overlay is visible — so the first tap after the overlay appears always lands.
...
#### Stale `sense_awake_confirmed` trust window (`SENSE_AWAKE_TRUST_MS`, 2026-06-17)

The Hard Gate `if (sense_awake_confirmed) { send_sense_ping(); return; }` suppressed every
GPIO39 wake pulse whenever the LCD *believed* the Sense was awake — but that belief could be
**stale**. When the Sense deep-sleeps it sends `SLEEP_READY` then `uart_drain_tx(150)`, flushing
queued `PONG`/`UI_STATUS`/`SYNC_ACK` **after** the sleep announcement. The LCD processes
`SLEEP_READY` (→ `SENSE_ASLEEP`, clears the flag), then processes those drained messages, which
re-set `sense_awake_confirmed = true`. Now the Sense is physically deep-asleep but the LCD thinks
it's awake, so the gate sends only a UART ping — which **cannot** wake a deep-sleeping Sense (only
the EXT0 GPIO2 pulse can). Symptom: Settings showed Sense fw `--` indefinitely.

**Fix — "trust but verify":** the awake flag is only trusted if we've *also* actually heard from
the Sense within `SENSE_AWAKE_TRUST_MS` (`1500` ms, defined in `LCD_Minimal.ino` next to
`SENSE_AWAKE_GRACE_MS`). `sense_recently_heard(SENSE_AWAKE_TRUST_MS)` (uses `last_sense_any_rx_ms`)
gates all three wake sites in `LCD_Minimal.ino`:
- `lcd_maybe_pulse_sense_int()` — if confirmed but RX stale: log, `sense_awake_confirmed = false`,
  `sense_state_set(SENSE_ASLEEP, "stale_awake_no_rx")`, fall through to the pulse.
- `wake_sense_for_request()` — same stale-clear, falls through to its pulse path.
- `request_sense_wake()` — only short-circuits when `sense_awake_confirmed && sense_recently_heard(...)`;
  otherwise falls through to `lcd_maybe_pulse_sense_int()` (which does the stale-clear + pulse).

`SENSE_AWAKE_TRUST_MS` is defined textually before all three functions. A genuinely-awake Sense
(recent RX) still takes the UART-only path — no spurious pulses.
...
| `EVT_USB_DELTOUCH` | (none) | USB `deltouch`: open the Delete/Back overlay for the current selection, then ~300ms later synthesize a tap at the center of the real Delete button (UI task) |
...
| `deltouch` | **Full touch-path delete** for the e2e harness — unlike `del N` (which calls `shopping_list_delete_index` directly), this exercises the real overlay + derived-hitbox path without a finger. Posts `EVT_USB_DELTOUCH`; the UI task opens the overlay for the current `shopping_list_scroll_idx` via `shopping_list_show_overlay()` (prints `[USB] deltouch -> overlay@idx`, sets `usb_deltouch_tap_due_ms = millis()+300`), then a deferred check in the UI-task loop synthesizes a tap at the CENTER of the actual Delete button rect (`lv_obj_get_coords`) through `shopping_list_handle_touch(cx, cy)` (prints `[USB] deltouch tap at center of Delete (cx,cy)`; prints `[USB] deltouch tap skipped (overlay gone)` if the overlay vanished in the interim). Assertions key off the `[SHOPPING_LIST] overlay tap (x,y) -> delete` classification log. Ignored off the list screen or with an empty list. |
| `liststate` | Posts `EVT_USB_LISTSTATE`; the UI task prints one machine-readable line for harness assertions: `[LISTSTATE] {"screen":N,"refresh_state":N,"pill":0\|1,"count":N,"cache_age_s":N,"auto_retry":N,"selected":N,"latch":0\|1,"armed":0\|1,"scroll_y":N,"pill_hiding":0\|1,"dedupe_age_ms":N}` (`dedupe_age_ms` = `millis() - lcd_last_ui_list_complete_ms`, capped at 99999, -1 if no UI_LIST has completed this boot — makes the UI_LIST dedup window observable; `cache_age_s` = -1 when unknown — no cache or no valid wall clock; `selected` = `shopping_list_scroll_idx`; `latch` = `shopping_list_touch_pull_consumed` — 1 after a completed refresh means the pull-to-refresh latch is wedged; `armed` = `shopping_list_touch_pull_armed`; `scroll_y` = `lv_obj_get_scroll_y(shopping_list_scroll)`, 0 if the container is null, negative = pulled below the top; `pill` = border refresh ring active/visible and `pill_hiding` = ring fade-out in progress — the field NAMES are kept from the old pill indicator for harness compatibility). |
...
11. **Shopping-list refresh in flight** — `refresh_state == REFRESH_WAKE_PENDING || REFRESH_INFLIGHT` blocks sleep (reason `refresh_inflight`) so a refresh can complete instead of the 10s idle-sleep firing ~10s into a slow-WiFi fetch. **Capped** at `REFRESH_KEEPAWAKE_MAX_MS` (30000) measured from the refresh start (`refresh_wake_pending_start_ms` if non-zero, else `lcd_refresh_start_ms`) so a wedged refresh can't pin the device awake forever.
12. WiFi connecting
13. User activity within timeout period
14. Not on a sleep-eligible screen (HOME, SECOND, SETTINGS, SHOPPING_LIST)
15. HOME screen shown less than `HOME_SLEEP_DELAY_MS` (10s) ago
16. Dish processing in progress
...
**Save/load symmetry is load-bearing.** Both paths use `DynamicJsonDocument(LIST_PERSIST_JSON_CAPACITY)` (16KB, heap-allocated so no task-stack pressure). The historical asymmetry — 8KB `StaticJsonDocument` on save vs 4KB on load — let large lists persist fine but fail to parse back with `NoMemory`, so the user saw a blank "No items on your list" after every deep sleep (LCD deep sleep is a full reset; `g_active` is RAM-only).

**Capacity bump 8192 → 16384 (2026-06-17).** Adding the `stores[]` array to the saved JSON (now `items[]` + `ids[]` + `stores[]`) pushed worst-case 50-item lists past the old 8192-byte doc, so `deserializeJson` failed on load (`NoMemory`) → `load_list_from_storage` returned -1 → `g_active.count` stayed 0 → "No items on your list" on entry until a manual pull. Bumped the shared `LIST_PERSIST_JSON_CAPACITY` constant (used by BOTH the save and load `DynamicJsonDocument`) to 16384. Worst-case math (50 items): 3 arrays ≈ 2.4KB + items 50×64 + ids 50×64 + stores 50×48 ≈ 8.8KB strings = ~11.2KB, fits 16384 with headroom for both serialize and deserialize. **OLD-cache robustness:** load now `doc.containsKey("stores")`-guards the stores array (absent key → empty `JsonArray()` → every store cleanly resolves to `""`) so pre-store caches still load instead of failing the whole parse; the per-index `(i < stores_array.size())` guard already handled short arrays. A successful load also logs `[LIST_CACHE] loaded %d items from NVS` for verification.

**Empty-save guard (cache-clobber protection).** `save_list_to_storage()` skips the save when `count == 0` **and** `g_list_refresh_completed_once` is false (no UI_LIST has completed a refresh this boot), logging `[LIST_PERSIST] skip empty save (no completed refresh this boot)`. This stops an empty in-memory list (boot race, transient empty UI_LIST, pre-sleep belt-and-braces save before any load) from clobbering a good non-empty NVS cache — the cause of "No items on your list" after the next deep sleep. Once a refresh has genuinely completed this boot, an empty save is allowed (the list may truly be empty). Edge case accepted: deleting the *last* cached item before any refresh completes this boot won't persist the now-empty list, so the item can reappear after sleep until the next refresh.
...
| Function | Description |
|----------|-------------|
| `save_list_to_storage(s)` | Serializes `app_state_t` to JSON, saves to NVS + `fetched_at` stamp; skips empty saves until `g_list_refresh_completed_once` (cache-clobber guard) |
