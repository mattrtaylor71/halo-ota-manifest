...
2b. **`ota_complete` cloud report (telemetry, exactly once per run):** immediately after the LCD proxy phase computes `lcd_proxy_succeeded`, and BEFORE the Sense self-OTA can `esp_restart()`, the device POSTs an `ota_complete` report via `ota_report_post("ota_complete", &extras, ...)`. This is the single chokepoint both branches pass through exactly once (it sits past the LCD retry loop and before `maybeRunOtaCheck()`), so the maintenance outcome is visible in the cloud even when the Sense self-OTA reboots and never returns. The payload carries the standard fields — `fw` (current Sense, NOT yet self-updated at this point), `lcd_fw`, `lcd_ota_result` (e.g. `end_ack_timeout` / `updated` / `noop`) — plus extras `sense_fw_target` (`OtaIntent::getDesiredSense()`, falling back to `truth_get_manifest_state().version`) and `lcd_ota_succeeded` (`"1"`/`"0"`). This is what diagnoses the Sense-ahead/LCD-behind asymmetry (cold UART → LCD-proxy `end_ack_timeout` while the Sense still self-updates). The `lcd_ota_due` boot-fallback branch at the top of `run_maintenance_if_needed()` does NOT emit this report (it has no Sense self-OTA).
